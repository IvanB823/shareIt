...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDto createBooking(Long userId, BookingCreateDto bookingDto) {
        User booker = userRepository.findById(userId)
                .orElseThrow(() -> new UserNotFoundException("Пользователь c данным id не найден"));

        Item item = itemRepository.findById(bookingDto.getItemId())
                .orElseThrow(() -> new ItemNotFoundException("Предмет c данным id не найден"));

        if (!item.getAvailable()) {
            throw new RuntimeException("Предмет недоступен для бронирования");
        }

        if (item.getOwner().getId().equals(userId)) {
            throw new AccessDeniedException("Собственник не может забронировать свою собственность");
        }

        if (bookingDto.getStart().isAfter(bookingDto.getEnd()) ||
                bookingDto.getStart().equals(bookingDto.getEnd())) {
            throw new ValidationException("Неверные даты бронирования вещи");
        }
    ...
    public BookingDto updateBookingStatus(Long userId, Long bookingId, Boolean approved) {
        ...
        if (booking.getStatus() != BookingStatus.WAITING) {
            throw new AccessDeniedException("Бронирование уже одобрено или отклонено собственником");
        }
